...
describe('isPortVar', () => {
  ...
  it('should detect port by numeric value in common dev port range (1000-9999)', () => {
    ...
    expect(isPortVar('REDIS_ADDR', '6379')).toBe(true);
  });
  ...
  it('should not detect non-port values', () => {
    ...
    expect(isPortVar('TIMEOUT_MS', '30000')).toBe(false); // above dev port range
    expect(isPortVar('BATCH_SIZE', '15000')).toBe(false); // above dev port range
    expect(isPortVar('MONGO_URI', '27017')).toBe(false);  // above dev port range (no "port" in key)
