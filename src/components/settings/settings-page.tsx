...
import { useSettings, useAuth } from '@/lib/query/hooks';
import { useSaveSettings, useSetupAuth, useImportProject } from '@/lib/query/mutations';
...
import { FolderOpen, Globe, Shield, Search, X, Check, AlertTriangle, Loader2, Power } from 'lucide-react';
...
export function SettingsPageContent() {
  ...
  const setupAuthMut = useSetupAuth();
  const importProjectMut = useImportProject();
  ...
  const saveSettings = async (updates: Record<string, string>) => {
    try {
      await saveSettingsMut.mutateAsync(updates);
  ...
  const handleImportProject = async (path: string) => {
    try {
      await importProjectMut.mutateAsync(path);
  ...
  const handleSetupPasscode = async () => {
    ...
    try {
      await setupAuthMut.mutateAsync(passcode);
  ...
  return (
    <div className="p-6 space-y-6 max-w-[720px]">
              ...
              {discoveredProjects.map((p, i) => (
                ...
                >
                  ...
                  {p.alreadyImported ? (
                    ...
                    <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={() => handleImportProject(p.path)}>Import</Button>
                  )}
                </div>
              ))}
            </div>
      ...
      {/* Shutdown Behavior */}
      <Card id="shutdown" className="relative overflow-hidden animate-fade-up" style={{ animationDelay: '225ms' }}>
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[40%] h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent" />
        <CardHeader>
          <div className="flex items-center gap-2">
            <Power className="w-[18px] h-[18px] text-muted-foreground/60" />
            <CardTitle className="text-[15px] font-semibold font-display">Shutdown Behavior</CardTitle>
          </div>
          <p className="text-xs text-muted-foreground">Control what happens when DevHub exits</p>
        </CardHeader>
        <CardContent>
          <div className="rounded-lg border border-border overflow-hidden">
            <div className={`px-4 py-3.5 flex items-center justify-between transition-colors duration-200 ${settings.stop_all_on_exit !== 'false' ? 'bg-primary/[0.03]' : ''}`}>
              <div>
                <Label className="text-sm">Stop all projects on exit</Label>
                <p className="text-xs text-muted-foreground">Terminate all running services when DevHub shuts down</p>
              </div>
              <Switch
                checked={settings.stop_all_on_exit !== 'false'}
                onCheckedChange={v => saveSettings({ stop_all_on_exit: String(v) })}
              />
            </div>
          </div>
        </CardContent>
