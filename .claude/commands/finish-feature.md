...
5. **Verification** (with Test Checklist for Reviewers)
   ...
   a) **Automated Tests**
      ...
      - Check test coverage (target: >80% for new code in `TypeScript` projects)
   ...
   b) **Manual Testing - Next.js (App Router) / Vitest**
      - For frontend (Next.js (App Router)): Render the component/page in browser, verify state updates work correctly, test all user interactions (clicks, form inputs, navigation), verify conditional rendering, check edge cases and error states
      - For backend/API (Next.js (App Router)): Test HTTP status codes and responses, verify request/response bodies match spec, test error cases and input validation, check database transactions/state, verify authentication/authorization if applicable
      - For CLI/desktop (Next.js (App Router)): Test command-line arguments and flags, verify output format and readability, test error messages and help text, check file I/O operations
      - Using Vitest framework: Verify test structure matches project conventions, assertions are clear and specific, setup/teardown is properly handled

   c) **Regression Testing**
      - Test related existing functionality to ensure no breakage
      - For Next.js (App Router) projects: check related components/endpoints/commands work correctly
   ...
   d) **Code Quality Checks**
      ...
      - Follows TypeScript conventions and style guide (see `CLAUDE.md`)
